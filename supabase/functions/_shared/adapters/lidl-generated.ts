...
export async function fetchLidlOffers(limit = 6): Promise<IngestedOffer[]> {
  try {
    const kupiOffers = await fetchKupiChainOffers("lidl", Math.max(limit, 10));

    if (kupiOffers.length > 0) {
      return kupiOffers.slice(0, limit);
    }
  } catch (error) {
    console.error("Lidl kupi adapter failed, falling back to leaflets", error);
  }
